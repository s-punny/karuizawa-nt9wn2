...
観光セクションの地図だけ Google マイマップの埋め込みを使っている。
複数のピンを1枚に立てるため（通常の埋め込みは1枚に1か所しか置けない）。
ピンの追加・修正はマイマップ側で行う。`index.html` は `mid` を持つだけ。
マイマップの共有が「リンクを知っている全員が閲覧可」でないと表示されないので、
共有設定を変えないこと。
